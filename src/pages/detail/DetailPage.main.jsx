...
const DetailPage = () => {
  ...
  const makeImageUrls = (imageUrls) => {
    return imageUrls.map((url, index) => `https://${url}`);
  };
  ...
  const handleLike = async () => {
    try {
      ...
      if (!isSaved) {
        await PostLike(restaurantDetail.restaurantId);
        updatedWishCount += 1;
        console.log("상세페이지에서 좋아요 성공!");
      } else {
        await DeleteLike(restaurantDetail.restaurantId);
        updatedWishCount -= 1;
  ...
  return (
    <style.TotalContainer>
      ...
      <style.InnerContentContainer>
        ...
        <style.MainInfoContainer>
          <style.MainInfoFirstContainer>
            ...
            <style.NameContainer>{restaurantDetail.name}</style.NameContainer>
          ...
          <style.MainInfoSecondContainer>
            ...
            <style.TimeContainer>
              ...
              {restaurantDetail.expandedDays === "null"
                ? "운영일 제공 x"
                : restaurantDetail.expandedDays}
              {"\u00A0\u00A0\u00A0"}
              {restaurantDetail.timeRange === "null"
                ? "운영시간 제공 x"
                : restaurantDetail.timeRange}
            </style.TimeContainer>
          </style.MainInfoSecondContainer>
        </style.MainInfoContainer>
        ...
        <style.MenuContainer>
          <style.MenuTitle>
            메뉴
            <style.MenuButton>메뉴판</style.MenuButton>
          </style.MenuTitle>
        ...
        <style.DetailInfoContainer>
          <DetailInfo
            cautions={restaurantDetail.cautions}
            convenience={restaurantDetail.facilities}
      ...
      <ReservationModal
        ...
        closeModal={closeModal}
        restaurantId={restaurantDetail.restaurantId}
        remoteSelectDate={remoteSelectDate}
      />
    </style.TotalContainer>
  );
};
